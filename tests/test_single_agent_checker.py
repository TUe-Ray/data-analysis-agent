    ...
    model = ScriptedRoleModel(
        {
            ...
            "final_checker": ['{"decision":"PASS","feedback":"Complete."}'],
        }
    )
...
def test_final_checker_allows_exactly_one_answer_repair(tmp_path: Path) -> None:
    model = ScriptedRoleModel(
        {
            "single_agent": [_generation(1), '{"value":2}'],
            "final_checker": [
                '{"decision":"REPAIR","feedback":'
...
def test_invalid_checker_repair_fails_without_a_second_repair(tmp_path: Path) -> None:
    model = ScriptedRoleModel(
        {
            "single_agent": [_generation(1), '{"value":"not an integer"}'],
            "final_checker": ['{"decision":"REPAIR","feedback":"Repair it."}'],
        }
    )
